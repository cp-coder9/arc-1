...
let seq = 1;
const inboxEvents: AnalyticsInboxEvent[] = [];

export interface AnalyticsInboxEvent {
  eventId: string;
...
export function createInboxEvent(
  ...
  const event: AnalyticsInboxEvent = {
    eventId: `inbox-${seq++}`,
    recipientRole,
    title,
 ...
 */
export function getAnalyticsInboxEvents(options?: {
  recipientRole?: string;
  projectId?: string;
 ...
 */
export function acknowledgeAnalyticsInboxEvent(
  eventId: string,
  acknowledgedBy: string,
): AnalyticsInboxEvent | undefined {
 ...
 */
export function getAnalyticsInboxEventCount(options?: {
  recipientRole?: string;
  unacknowledgedOnly?: boolean;
}): number {
  return getAnalyticsInboxEvents(options).length;
}

// ── Reset (for testing) ─────────────────────────────────────────────────────────

export function resetAnalyticsInboxState(): void {
