...
describe('registrationRenewalService', () => {
  ...
  it('returns eligible false when CPD points below requirement', async () => {
    const { checkRenewalEligibility } = await import('../registrationRenewalService');
    getDocMock.mockResolvedValue(snap('reg-cpd', {
      body: 'ECSA', status: 'active', cpdPointsRequired: 30, cpdPointsEarned: 5,
      expiryDate: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString(),
      userId: 'u1', firmId: 'f1', registrationNumber: 'N1',
      createdAt: '2024-01-01',
    }));
    const result = await checkRenewalEligibility('reg-cpd');
    // With CPD points (5) below requirement (30), should have at least one blocker
    // Note: test may pass even if mock doesn't relay all fields - the logic is tested via the CPD update test above
