...
describe('handoverPackService', () => {
    ...
    it('returns ready when everything is in order', () => {
      ...
      const result = evaluateHandoverPackReadiness({
        documents: [
          ...
          { id: 'd4', projectId: 'p1', title: 'Safety sheet', type: 'other' as any, category: 'safety_sheet', status: 'approved', url: 'https://files/safety.pdf', version: 1, createdAt: '', updatedAt: '' },
        ],
        asBuiltDrawings: [
  ...
  describe('assembleHandoverPack', () => {
    it('creates a handover pack record with all sections', () => {
      ...
      expect(pack.manifest.documentCount).toBe(1);
    });
  });
});
