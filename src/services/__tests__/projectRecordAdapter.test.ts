...
describe('projectRecordAdapter', () => {
  ...
  describe('toProjectRecord', () => {
    it('converts a WorkflowRecord to ProjectRecord', () => {
      ...
      expect(record.recordId).toMatch(/^project-record-/);
    ...
    it('attaches linked record IDs', () => {
      ...
      const record = toProjectRecord(ctx, wfRecord, ['linked-1', 'linked-2']);
  ...
  describe('storeAllKPIMetrics', () => {
    it('stores all KPI results from a computation', () => {
      ...
      const metrics = storeAllKPIMetrics(kpiResults, ctx);
  ...
  describe('getKPIMetrics', () => {
    it('filters by project', () => {
      ...
      const metrics = getKPIMetrics({ projectId: 'p1' });
    ...
    it('filters by KPI name', () => {
      ...
      const metrics = getKPIMetrics({ name: 'compliance_gap_count' });
      expect(metrics).toHaveLength(1);
    });
  });
  ...
  describe('getProjectRecord', () => {
    it('finds a record by ID', () => {
      ...
      const found = getProjectRecord(record.recordId);
      expect(found?.title).toBe('Note');
    });

    it('returns undefined for non-existent record', () => {
      expect(getProjectRecord('nonexistent')).toBeUndefined();
    });
  });
  ...
  describe('getProjectRecords', () => {
    it('returns all records for a project', () => {
      ...
      toProjectRecord(ctx, wf1);
      toProjectRecord({ ...ctx, projectId: 'other-project' }, wf2);

      const records = getProjectRecords('project-1');
