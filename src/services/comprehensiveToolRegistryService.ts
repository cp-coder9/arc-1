...
export const COMPREHENSIVE_TOOL_REGISTRY: ToolDefinition[] = [
  {
    ...
    benchmarkInspiration: ['ClickUp forms/docs', 'Architex Client/Architect dashboards'],
    existingArchitexHooks: ['ClientDashboard', 'ArchitectDashboard', 'GuidedBriefWizard'],
  ...
  },
  {
    ...
    existingArchitexHooks: ['FileManager', 'DrawingRegisterPage', 'documentCdeService'],
  ...
  },
  {
    ...
    benchmarkInspiration: ['RIB Candy', 'BuildSmart', 'GoBuild360', 'Architex TenderWizard/BidSubmission'],
  ...
  },
  {
    ...
    existingArchitexHooks: ['paymentService', 'financialLedgerService', 'InvoiceManagement', 'escrowGovernanceService'],
    requiresHumanApproval: true,
  ...
  },
  {
    ...
    category: 'proposal',
    description: 'Builds professional fee proposals with scope, terms, conditions, discount rules and appointment workflow.',
    roles: ['architect', 'bep', 'engineer', 'quantity_surveyor', 'town_planner'],
    phases: ['proposal_appointment'],
    exportTargets: ['invoice', 'task'],
    benchmarkInspiration: ['Fresh Projects fee proposals', 'Architex FeeEstimator/proposalBuilderService'],
    existingArchitexHooks: ['FeeEstimator', 'proposalBuilderService', 'feeEstimatorService'],
    requiresHumanApproval: true,
    southAfricanContext: ['SACAP/ECSA fee guidelines', 'professional appointment terms', 'VAT'],
  },
];

export function getToolsForContext(context: ToolContext): ToolDefinition[] {
  return COMPREHENSIVE_TOOL_REGISTRY.filter(
    (tool) => tool.roles.includes(context.role) && tool.phases.includes(context.phase),
  );
}
...
export function phaseToolSummary(role: ArchitexComprehensiveRole): Record<ArchitexWorkflowPhase, string[]> {
  const phases: ArchitexWorkflowPhase[] = [
    'lead', 'brief_feasibility', 'proposal_appointment', 'design_coordination',
    'municipal_submission', 'tender_procurement', 'construction_execution',
    'payments_commercial_control', 'closeout', 'operations_post_occupancy',
  ];
  return Object.fromEntries(
    phases.map((phase) => [
      phase,
      getToolsForContext({ userId: 'summary', role, phase }).map((tool) => tool.id),
    ]),
  ) as Record<ArchitexWorkflowPhase, string[]>;
}
