      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-border transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col p-6">
          ...
          <div className="space-y-2 flex-1">
            ...
            <NavItem icon={<FileText size={20} />} label="Compliance Vault" active={activeTab === 'files'} onClick={() => setActiveTab('files')} />
            <NavItem icon={<CreditCard size={20} />} label="Payments" active={activeTab === 'invoices'} onClick={() => setActiveTab('invoices')} />
            <NavItem icon={<Settings2 size={20} />} label="Settings" active={activeTab === 'profile-settings'} onClick={() => setActiveTab('profile-settings')} />
          </div>
