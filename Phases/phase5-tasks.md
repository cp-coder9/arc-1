...
- [ ] **Task 5.1** — Extend Escrow & Payment types in `src/types.ts`
  - `EscrowMilestone` interface
  - `EscrowV2` interface (extends Escrow)
  - `LedgerEntry` interface
  - Run `npm run lint`

- [ ] **Task 5.2** — Create `src/services/financialLedgerService.ts`
  - `recordTransaction()`
  - `getLedgerForProject()`
  - `getLedgerForUser()`
  - `getPlatformSummary()`
...
- [ ] **Task 5.3** — Stage-linked escrow in `src/services/paymentService.ts`
...
- [ ] **Task 5.4** — Create `src/components/FinancialDashboard.tsx`
  - Summary stat cards
  - Ledger table with filters
  - Revenue chart (CSS-based)
  - Escrow overview per project

- [ ] **Task 5.5** — Add "Financial" tab to `AdminDashboard.tsx`
...
- [ ] **Task 5.7** — Fee Estimator milestone breakdown
