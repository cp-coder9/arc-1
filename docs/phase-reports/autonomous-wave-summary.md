...
|---|---|---|
| `26f9e814` | Schema drift tests | Added a comprehensive runtime schema enum regression that keeps Zod domain enums aligned with TypeScript unions for roles, jobs, applications, submissions, notifications, payments, verification, AI review disciplines, responsible parties, risk statuses, and execution modes. |
| `16433065` | Runtime schemas | Aligned runtime application status validation with dashboard-supported `withdrawn` applications and added a schema regression. |
| `a724b435` | Notification schemas | Added invitation notification metadata fields to runtime schemas and schema regressions for invitation/project/package sender metadata. |
| `537adb03` | Permission tests | Added a schema/permission drift regression to keep runtime role validation aligned with canonical authorization roles. |
| `efd3c4b9` | Notification typing | Aligned `directory_invitation` notifications across shared types, runtime schemas, notification service config, bell icon rendering, and focused tests. |
| `80f5652f` | Verification tests | Added active-verification expiry edge regressions for no-expiry and invalid-expiry records. |
